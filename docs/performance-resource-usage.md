...
from old Roboco**, and **33.3% from the previous optimized build**.
...
| Metric | Old Roboco | Previous optimized | Final |
| --- | ---: | ---: | ---: |
...
cache. Final idle RSS remains 37.7% below old Roboco.
...
the driver requires exactly one Roboco window.

```sh
cargo build --release -p roboco
Xvfb :98 -screen 0 1440x900x24 -nolisten tcp
# In another terminal:
DISPLAY=:98 WAYLAND_DISPLAY= LP_NUM_THREADS=4 ROBOCO_FRAME_STATS=0 \
  ROBOCO_PROFILE_PSS=1 ROBOCO_PROFILE_IDLE_MS=45000 \
  CLAUDE_CODE_EXECUTABLE="$PWD/scripts/replay-claude.py" \
  ROBOCO_REPLAY_JOURNAL="$PWD/scripts/fixtures/resource-stream.jsonl" \
  node scripts/resource-profile.mjs target/release/roboco /tmp/roboco-short claude-code

# Use LP_NUM_THREADS=1 for the one-worker check.
# Add ROBOCO_REPLAY_REPEAT=10 ROBOCO_REPLAY_DELAY_MS=8 for the long workload.
# For a live turn, omit replay variables and point CLAUDE_CODE_EXECUTABLE to
# the authenticated CLI; ROBOCO_PROFILE_SUBMIT_UI=1 submits through the composer.
