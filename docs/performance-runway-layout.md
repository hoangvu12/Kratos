...
Build each revision with `cargo build --release --locked -p roboco`, copy each
...
DISPLAY=:108 WAYLAND_DISPLAY= LP_NUM_THREADS=4 ROBOCO_FRAME_STATS=0 \
  ROBOCO_PROFILE_PSS=1 ROBOCO_PROFILE_SUBMIT_UI=1 \
  ROBOCO_PROFILE_PRE_IDLE_MS=10000 ROBOCO_PROFILE_IDLE_MS=45000 \
  ROBOCO_REPLAY_DELAY_MS=40 \
