...
Roboco themes are complete, source-neutral `ThemeVariant` values owned by the
`roboco-theme` crate. Runtime components consume only Roboco semantic roles. VS
...
- Every variant records a recommended `SurfaceTreatment`: Roboco recommends
...
instead of fixed Roboco greys. Its window tint becomes denser when necessary to
...
still allowing the independent Roboco surface preference to force frost.

After source mapping, a deterministic hardening pass checks Roboco's shared
...
related source role when the preferred one cannot carry the Roboco role, and
...
The `roboco-theme-import` development tool exposes the same single-file adapter
for built-in curation:

Example:

```bash
cargo run -p roboco-theme --bin roboco-theme-import -- \
  --input /path/to/theme.json \
  --output /tmp/theme.roboco.json \
...
source. Roboco does not fetch or execute VSIX packages at runtime; custom
