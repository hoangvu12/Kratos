...
- Run `cargo fmt --check`, the relevant `roboco-ui` tests/checks, and a local app build.
...
- `cargo check -p roboco-ui` passes with existing workspace warnings.
- `cargo test -p roboco-ui --lib settings::appearance` passes (4 tests).
...
- Product goal: capture the frontmost application from a global shortcut and
  stage screenshot plus semantic application context in a Roboco composer.
...
- Export behavior: none. A send reuses Roboco's attachment upload and prompt
  transport.
...
- Added an isolated `Roboco Dev.app` workflow with bundle ID
  `sh.roboco.app.dev`, stable Apple Development signing, its own data directory,
  and IPC port. It launches through LaunchServices so TCC attributes capture
  permissions to Roboco Dev rather than the terminal.
...
- Verification tier: pure tray-height and prompt tests, full `roboco-ui` unit
...
- Verification tier: pure PNG/aspect-ratio tests, full `roboco-ui` unit suite,
...
- Wayland decision: probe portal versions and advertised targets. Prefer
  ...
  Shortcuts portal and expose `roboco appshot` for system-managed shortcut
