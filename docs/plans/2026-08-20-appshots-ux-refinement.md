...
4. Update pure layout tests and permission-facing copy. Validate with
   `cargo test -p roboco-ui --lib`, `cargo check -p roboco-ui`,
   `cargo build -p roboco`, and `git diff --check`.
...
4. Add dimension-parser and landscape/portrait/square sizing tests, run the
   `roboco-ui` test suite and macOS build checks, then inspect the result in the
   signed `Roboco Dev.app`.
