...
2. Add `crates/ui/src/appshots/windows.rs` using `RegisterHotKey`,
   ...
   to screenshot-only or a concrete capture error; Roboco does not request
   `UIAccess`.
...
4. Add a Linux-only `roboco appshot` activation command and local activation
...
- `cargo fmt --all -- --check`, `cargo test -p roboco-ui --lib`,
  `cargo check -p roboco-ui`, and `cargo check -p roboco` on macOS.
