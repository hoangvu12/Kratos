...
Build each revision with `cargo build --release --locked -p roboco` and copy the
...
DISPLAY=:108 WAYLAND_DISPLAY= LP_NUM_THREADS=4 ROBOCO_FRAME_STATS=0 \
  ROBOCO_PROFILE_PSS=1 ROBOCO_PROFILE_SUBMIT_UI=1 ROBOCO_PROFILE_IDLE_MS=45000 \
  CLAUDE_CODE_EXECUTABLE="$PWD/scripts/replay-claude.py" \
  ROBOCO_REPLAY_JOURNAL="$PWD/scripts/fixtures/resource-stream.jsonl" \
  node scripts/resource-profile.mjs /path/to/roboco /tmp/fresh-run claude-code
```

For the short workload, use `scripts/fixtures/runway-short-stream.jsonl`, set
`ROBOCO_REPLAY_DELAY_MS=400`, `ROBOCO_PROFILE_PRE_IDLE_MS=5000` and
`ROBOCO_PROFILE_IDLE_MS=20000`. The replay makes no model API calls.
