...
```sh
cargo test --release --locked -p roboco-ui --lib -- --test-threads=1
cargo test --release --locked -p roboco-engine --lib --test workspace_files --test device_routing
```
...
No zui dependency changes are required. Roboco uses `on_pinch` and `on_scroll_wheel` from its existing pinned revision.
