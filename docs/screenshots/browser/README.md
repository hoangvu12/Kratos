...
which runs Roboco's shell with synthetic local chat data and a loopback website.
...
```sh
cargo run --release --locked -p roboco-ui --example browser-fixture \
...
passed. The hosted Mac has a 1024px desktop, so the fixture uses Roboco's existing
