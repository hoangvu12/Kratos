...
labels, while retaining Roboco's Geist fonts and desktop palette. Code line
...
xcodebuild -project apps/ios/Roboco.xcodeproj -scheme Roboco \
  -destination 'platform=iOS Simulator,name=iPhone 17 Pro' test
```

`RobocoTests/TranscriptFollowTests` checks follow state independently of layout.
`RobocoTests/TranscriptLayoutTests` mounts the real SwiftUI transcript in a
...
`RobocoUITests/MobilePolishTests` exercises actual project selection/dismissal,
