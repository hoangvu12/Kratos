...
With `ROBOCO_VERIFY_CACHE=1`, the example also compares the reused scene with a
...
`ROBOCO_VERIFY_INTERACTIONS=1` also drives a text-selection drag, composer typing,
...
ROBOCO_FRAME_STATS=0 ROBOCO_PROFILE_IDLE_MS=10000 \
  CLAUDE_CODE_EXECUTABLE="$PWD/scripts/replay-claude.py" \
  ROBOCO_REPLAY_JOURNAL="$PWD/scripts/fixtures/resource-stream.jsonl" \
  node scripts/resource-profile.mjs target/release/roboco /tmp/roboco-native claude-code

# UI-only offscreen replay of those verified protocol frames:
cargo build --release --locked -p roboco-ui --features resource-profile \
  --example macos-resource-profile
ROBOCO_VERIFY_CACHE=1 ROBOCO_VERIFY_INTERACTIONS=1 ROBOCO_FRAME_STATS=0 \
  target/release/examples/macos-resource-profile \
  /tmp/roboco-native/frames.json /tmp/roboco-native-ui

# Larger/faster reply: use the foreground command above with these added:
# ROBOCO_REPLAY_REPEAT=4 ROBOCO_REPLAY_DELAY_MS=10 ROBOCO_PROFILE_IDLE_MS=30000
# and a fresh output directory.

# Native counter usable with either process (CPU uses 100% per core):
xcrun clang -O2 scripts/macos-resource-stat.c -o /tmp/roboco-stat
/tmp/roboco-stat PID
```
