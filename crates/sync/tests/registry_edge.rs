...
//!
//! ```sh
//! ROBOCO_EDGE_WS=ws://127.0.0.1:27640 cargo test -p roboco-sync --test registry_edge -- --ignored
//! ```
...
use roboco_doc::RegistryDoc;
use roboco_proto::{Chat, Session, SessionStatus};
use roboco_sync::RegistryClient;
...
fn edge_url(org: &str, user: &str) -> String {
    let base = std::env::var("ROBOCO_EDGE_WS")
        .expect("set ROBOCO_EDGE_WS to the edge origin, e.g. ws://127.0.0.1:27640");
...
#[tokio::test]
#[ignore = "requires a live edge: set ROBOCO_EDGE_WS (e.g. ws://127.0.0.1:27640)"]
