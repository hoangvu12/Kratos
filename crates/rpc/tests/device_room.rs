...
fn noop_nudge() -> roboco_rpc::NudgeHandler {
    Arc::new(|_| {})
}
...
async fn nudges_reach_the_host_callback() {
    ...
    let on_nudge: roboco_rpc::NudgeHandler = Arc::new(move |chat_id| {
...
/// `ROBOCO_EDGE_WS=http://127.0.0.1:26640 cargo test -p roboco-rpc -- --ignored live_edge`
/// (dev-mode edge; ROBOCO_EDGE_TOKEN defaults to a fixed dev user id).
#[tokio::test]
#[ignore = "needs a running edge (set ROBOCO_EDGE_WS)"]
async fn live_edge_relay_round_trip() {
    let Ok(edge_url) = std::env::var("ROBOCO_EDGE_WS") else {
        panic!("set ROBOCO_EDGE_WS to the edge base URL (e.g. http://127.0.0.1:26640)");
    };
    let token = std::env::var("ROBOCO_EDGE_TOKEN").unwrap_or_else(|_| "relay-live-test".into());
...
async fn zombie_relay_path_trips_the_echo_deadline() {
    ...
    // deadline instead.
    roboco_rpc::device_room::set_client_liveness_for_tests(
    ...
    roboco_rpc::device_room::set_client_liveness_for_tests(Duration::ZERO, Duration::ZERO);
}
