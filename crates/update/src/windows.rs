...
const CONFIG: &str = "roboco-update.json";
...
const BACKUP: &str = "roboco.exe.old";
...
const INCOMING: &str = ".roboco-update-incoming.exe";
...
pub(super) fn is_managed(exe: &Path) -> bool {
    exe.file_name().is_some_and(|name| name == "roboco.exe")
...
pub fn artifact(version: &str) -> String {
    format!("roboco-{version}-windows-{}.exe", std::env::consts::ARCH)
}
...
pub async fn stage(
    ...
    let temporary = tempfile::Builder::new()
        .prefix(".roboco-update-")
        .tempdir_in(directory)?;
    let staged = temporary.path().join("roboco.exe");
    ...
    .context("staged executable version check timed out")??;
        ...
        output.status.success()
            && String::from_utf8_lossy(&output.stdout).trim()
                == format!("roboco {}", manifest.version),
...
pub fn apply(staged: &Path, directory: &Path, relaunch: bool) -> anyhow::Result<()> {
    let installed = directory.join("roboco.exe");
    ensure!(
...
mod tests {
    ...
    /// A staged update layout: `<dir>/roboco.exe` plus its `sha256` sidecar.
    fn staged_with(dir: &Path, bytes: &[u8]) -> PathBuf {
        let staged = dir.join("roboco.exe");
    ...
    fn portable_install_requires_explicit_configuration() {
        ...
        let exe = dir.path().join("roboco.exe");
    ...
    fn failed_swap_after_old_image_moved_restores_the_installation() {
        ...
        let installed = install.path().join("roboco.exe");
    ...
    async fn corrupt_download_preserves_install_and_removes_staging() {
        ...
        let dir = tempfile::tempdir().unwrap();
        let installed = dir.path().join("roboco.exe");
