...
async fn authenticated_coordinator_pairs_devices_for_large_http_preview() {
    let edge = std::env::var("ROBOCO_PREVIEW_TEST_EDGE")
        .expect("set ROBOCO_PREVIEW_TEST_EDGE to the local dev Worker");
    tokio::time::timeout(Duration::from_secs(60), async {
        ...
        host.replace_local(vec![("/work/project".into(),Listener { pid:std::process::id(),parent:1,cwd:"/work/project".into(),args:vec!["node".into(),"vite".into()],started_at:1,address,roboco_owned:true })]).unwrap();
