//! Animation kit — the roboco motion catalog as reusable helpers over gpui
...
//! - `roboco-pulse` 2.4s staggered cell opacity 0.08→1, scale 0.9→1 (loaders)
...
/// Roboco-pulse floor opacity.
// The loader constants and math live in `roboco_proto::motion` (pure phase
...
// the pointer enters. The original roboco puts Tailwind `transition-colors`
...
/// Dev/measurement knob (`ROBOCO_MOTION_SCALE`, default 1): stretches every
/// catalog timeline by this factor — e.g. `ROBOCO_MOTION_SCALE=10` slows the
...
pub fn speed_scale() -> f32 {
    ...
    *SCALE.get_or_init(|| {
        std::env::var("ROBOCO_MOTION_SCALE")
            .ok()
...
mod tests {
    ...
    #[test]
    fn catalog_timings_match_roboco() {
        ...
        assert_eq!(ROBOCO_PULSE.duration_ms, 2400);
