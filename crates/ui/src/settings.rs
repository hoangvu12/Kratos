...
//! collapse flags (roboco persisted the same set in localStorage).
//!
...
/// Copy a selected image into Roboco's device-local data directory and make it
...
pub struct UiSettings {
    ...
    /// Master switch for session notification chimes. `ROBOCO_DISABLE_SOUND`
    ...
    /// `ROBOCO_DISABLE_NOTIFICATIONS` overrides.
    pub notifications_enabled: bool,
    /// Suppress the banner while a Roboco window is focused (the chime covers
    /// the foreground case).
    pub notifications_background_only: bool,
    pub right_pane_width: f32,
    /// Legacy: panel *open* flags are session-scoped in-memory state now
    /// (`shell::SessionPanels`, roboco `sessionPanels` parity). Kept for file
    ...
    pub accent: roboco_theme::AccentSelection,
    /// Glass policy, independent from the selected appearance, theme, and accent.
    pub surface: roboco_theme::SurfacePreference,
...
impl ShortcutId {
    ...
    /// Row label (roboco lib/shortcuts.ts `SHORTCUT_DEFINITIONS`, verbatim).
...
impl UiSettings {
    ...
    fn migrated(mut self) -> Self {
        if self.accent == roboco_theme::AccentSelection::ThemeDefault
            && let Some(accent) = self.legacy_accent_color.take()
        {
            self.accent = roboco_theme::AccentSelection::Preset(accent.into());
        }
...
mod tests {
    ...
    fn composer_send_behavior_is_opt_in_for_old_and_partial_settings() {
        ...
        assert!(loaded.open_web_links_in_roboco);
    ...
    fn round_trip() {
        ...
        let settings = UiSettings {
            ...
            theme_selection: roboco_theme::ThemeSelection {
    ...
    fn legacy_accent_color_migrates_to_an_explicit_preset() {
        ...
        assert_eq!(
            loaded.accent,
            roboco_theme::AccentSelection::Preset(roboco_theme::AccentPreset::Cyan)
        );
