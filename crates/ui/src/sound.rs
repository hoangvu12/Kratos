...
//! - `ROBOCO_DISABLE_SOUND` env kill-switch + the `soundEnabled` ui-setting;
...
const DISABLE_ENV: &str = "ROBOCO_DISABLE_SOUND";
...
fn create_temp_file() -> std::io::Result<(std::fs::File, TempSoundFile)> {
    for _ in 0..128 {
        ...
        let path =
            std::env::temp_dir().join(format!("roboco-sound-{}-{id}.wav", std::process::id()));
...
fn run_player(path: &Path) -> Result<(), String> {
    ...
    let script = "(New-Object Media.SoundPlayer $env:ROBOCO_SOUND_PATH).PlaySync()";
    ...
    pub(crate) fn new(session: &Session, now: chrono::DateTime<chrono::Utc>) -> Self {
        Self {
            ...
            fresh: now
                ...
                .num_milliseconds()
                <= roboco_proto::view::SESSION_STALE_MS,
        }
    }
...
pub(crate) fn connectivity_sound_since(
    current: roboco_proto::ConnectivityState,
    previous: roboco_proto::ConnectivityState,
) -> Option<Sound> {
    use roboco_proto::ConnectivityState as State;
...
pub(crate) struct ConnectivityNotificationState {
    previous: Option<roboco_proto::ConnectivityState>,
