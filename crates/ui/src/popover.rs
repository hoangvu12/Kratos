...
use crate::motion::{self, ROBOCO_PULSE};
...
/// The floating-menu surface (roboco `.glass-surface` + `menuSurface`):
...
/// One menu row (roboco `menuItem`): `gap-2.5 rounded-lg px-2 py-1.5
...
/// `bg-white/[0.08]` (roboco's `data-[highlighted]` styling) — two selected-
...
/// Small uppercase section heading inside a floating menu (roboco
...
/// Hairline divider between menu sections (roboco `MenuSeparator`:
...
/// The search/text input frame at the top of a picker popover (roboco
...
/// A bordered trailing menu section (roboco picker action groups /
...
// ---------------------------------------------------------------------------
// Dialog primitives (roboco dialog.tsx / sidebar dialogs.tsx)
// ---------------------------------------------------------------------------
...
/// `transition-colors` (roboco dialogs.tsx). Caller adds id + click; `fade_key`
/// as in [`menu_row`].
...
/// Pulsing skeleton rows shown while a list loads (roboco:
