...
impl Shell {
    pub fn new(state: Entity<AppState>, boot: EngineBootConfig, cx: &mut Context<Self>) -> Self {
        ...
        // `ROBOCO_FORCE_GATE=failed` renders that gate regardless of
        // engine connection state (display-only — for styling passes).
    ...
    fn copy_roboco_conversation_link(&mut self, chat_id: &str, cx: &mut Context<Self>) {
        let link = {
            let state = self.state.read(cx);
            crate::links::workspace_locator(state.local_device_id.as_deref())
