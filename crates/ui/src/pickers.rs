...
use roboco_engine::registry::HarnessDescriptor;
use roboco_proto::{
...
/// (headless-rig captures; same family as `ROBOCO_OPEN_PICKER`).
fn slow_catalog_delay() -> Option<std::time::Duration> {
    std::env::var("ROBOCO_SLOW_CATALOG_MS")
        .ok()
...
pub enum CheckoutPlan {
    ...
    /// `CreateWorktree` off `base` on send (roboco mints a `roboco/<name>`
...
/// lead with the flagship — roboco's `pickDefaultModel` Opus preference maps to
/// the same row here).
pub fn default_model(models: &[Model]) -> Option<&Model> {
    models.first()
}

/// A model's default reasoning: X-High when the ladder offers it (roboco
...
/// or foreign level — roboco use-run-config.ts's derived-model discipline).
...
pub fn browser_rows(listing: &FolderListing) -> Vec<&roboco_proto::FolderEntry> {
...
pub struct Pickers {
    ...
    config: DraftConfig,
    /// Sticky last-used picks (roboco `roboco.composer.defaults:v1`): seeds the
    ...
    focus: FocusHandle,
    /// `ROBOCO_OPEN_PICKER` boot: keep claiming focus until it sticks, so
...
impl Pickers {
    pub fn new(state: Entity<AppState>, cx: &mut Context<Self>) -> Self {
        ...
        // Dev/testing knob: `ROBOCO_OPEN_PICKER=model|traits|repo|branch` boots
        // with that popover open — synthetic input can't reach the app on
        // headless compositors, so captures need a data-side path.
        let boot_open = match std::env::var("ROBOCO_OPEN_PICKER").ok().as_deref() {
    ...
    fn effective_harness(&self, cx: &App) -> Option<HarnessId> {
        ...
        // model (it stays available under `ROBOCO_HARNESS=mock`).
        self.harnesses
            .ready()
    ...
    /// Capture knob (`ROBOCO_OPEN_DIALOG=model`): open the combined
    ...
    /// picker's rail + list panes bleed to the card edge (roboco
...
pub(crate) fn normalize_model_rows(harness: HarnessId, models: Vec<Model>) -> Vec<Model> {
    ...
    let catalog = match harness {
        HarnessId::ClaudeCode => roboco_harness::claude::catalog::static_models(),
        _ => Vec::new(),
    };
        ...
        .filter_map(|mut model| {
            ...
            if let Some(base) = strip_1m(&model.id.clone()) {
                ...
                if !model.options.iter().any(|o| o.id == "contextWindow") {
                    model.options.push(roboco_proto::ModelOption {
...
/// `ROBOCO_HARNESS=mock` (the e2e/dev rig) opts the mock harness into the UI;
/// production launches never set it, so the mock never surfaces there.
fn mock_harness_enabled() -> bool {
    std::env::var("ROBOCO_HARNESS")
        .ok()
        .as_deref()
        .map(str::trim)
        == Some("mock")
...
/// `ROBOCO_HARNESS=mock` shows it; otherwise it only remains when it's
...
fn offered_harnesses_impl(list: &[HarnessDescriptor], allow_mock: bool) -> Vec<HarnessDescriptor> {
    visible_harnesses_impl(list, allow_mock)
        ...
        .filter(|d| {
            d.installed
                && (roboco_engine::registry::descriptor_enabled(d)
...
impl Render for Pickers {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        // A popover opened data-side (ROBOCO_OPEN_PICKER) never went through
        ...
        // Chip shows the model's display name alone (roboco `modelText`); the
...
mod tests {
    use super::*;
    use roboco_proto::{FolderEntry, Model, ModelOption, ModelOptionChoice};
    ...
    fn browser_navigation_reducer() {
        ...
        assert_eq!(browser_rows(&listing)[1].name, "roboco");
    }
    ...
    fn mock_harness_hidden_unless_alone() {
        ...
        // …and opted back in by ROBOCO_HARNESS=mock (the e2e rig).
