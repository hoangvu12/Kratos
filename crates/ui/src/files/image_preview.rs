        ...
        self.task = Some(cx.spawn(async move |this, cx| {
            ...
            let load = async {
                ...
                let checkout = match context.checkout_id.clone().filter(|id| !id.is_empty()) {
                    ...
                    None => {
                        client
                            .read_file(roboco_proto::ReadWorkspaceFileRequest {
...
mod tests {
    ...
    impl super::super::client::WorkspaceFilesTransport for PendingTransport {
        async fn call(
            ...
            _: &str,
            _: serde_json::Value,
        ) -> Result<serde_json::Value, roboco_rpc::RpcError> {
        ...
        async fn subscribe(
            ...
            _: &str,
            _: serde_json::Value,
        ) -> Result<tokio::sync::mpsc::Receiver<serde_json::Value>, roboco_rpc::RpcError> {
            unreachable!()
        }
    }
    ...
    impl super::super::client::WorkspaceFilesTransport for ImageTransport {
        async fn call(
            ...
            if method == roboco_rpc::methods::READ_WORKSPACE_FILE {
                return Ok(
                    serde_json::json!({ "checkoutId": "checkout", "path": "remote.svg", "size": 0, "encoding": "binary", "truncated": false }),
                );
            }
            assert_eq!(method, roboco_rpc::methods::READ_WORKSPACE_IMAGE);
    ...
    fn image_preview_loads_through_owner_and_resolves_legacy_identity(
        ...
        for checkout_id in [Some("checkout".to_string()), None] {
            ...
            assert_eq!(
                calls.last().unwrap().0,
                roboco_rpc::methods::READ_WORKSPACE_IMAGE
            );
        }
    }
    #[cfg(target_os = "linux")]
