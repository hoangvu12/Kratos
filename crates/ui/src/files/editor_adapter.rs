//! Roboco-owned styling and highlighting adapters for `gpui-base`.
...
use roboco_syntax::{HighlightKind, HighlightedDocument};

use super::editor::FileEditorState;
use crate::theme::{SyntaxPalette, Theme};

#[derive(Clone)]
struct RobocoHighlightStyleResolver {
    palette: SyntaxPalette,
}

impl HighlightStyleResolver for RobocoHighlightStyleResolver {
...
#[derive(Clone)]
pub(super) struct RobocoInputHighlighter {
    language: SharedString,
    spans: Vec<(Range<usize>, HighlightKind)>,
}

impl RobocoInputHighlighter {
...
impl InputHighlighter for RobocoInputHighlighter {
...
pub(super) fn editor_style(theme: &Theme) -> InputEditorStyle {
    InputEditorStyle {
        ...
        caret: theme.caret,
        highlight_styles: Arc::new(RobocoHighlightStyleResolver {
...
pub(super) fn install_highlighter(
    ...
    editor.update(cx, |state, cx| {
        state.set_highlighter_factory(
            Rc::new(move |_| {
                Some(Box::new(RobocoInputHighlighter::new(&source, &document)) as Box<_>)
            }),
            cx,
        );
    });
...
mod tests {
    use super::*;
    use roboco_syntax::{HighlightSpan, LanguageId};
    ...
    fn style_runs_are_ordered_non_overlapping_and_cover_the_request() {
        ...
        let highlighter = RobocoInputHighlighter::new(source, &document);
        let resolver = RobocoHighlightStyleResolver {
    ...
    fn stale_highlight_ranges_are_rejected_before_updated_unicode_is_shaped() {
        ...
        let highlighter = RobocoInputHighlighter::new(stale_source, &stale_document);
        let resolver = RobocoHighlightStyleResolver {
            palette: Theme::dark().syntax,
        };
        let updated_text = "é";
