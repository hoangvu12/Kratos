...
//!
//! roboco never called `cx.set_menus`, so on macOS `NSApp.mainMenu` stayed nil:
...
pub fn init(cx: &mut App) {
    ...
    // Window verbs route to the active window. roboco is single-window, so a
...
/// The roboco menu bar. macOS renders this natively; mac-only entries are gated
...
pub fn app_menus() -> Vec<Menu> {
    ...
    let mut app_items = vec![
        ...
        MenuItem::action("About Roboco", About).disabled(!macos),
        MenuItem::separator(),
    ...
    if macos {
        app_items.extend([
            ...
            MenuItem::separator(),
            MenuItem::action("Hide Roboco", Hide),
            MenuItem::action("Hide Others", HideOthers),
            MenuItem::action("Show All", ShowAll),
            MenuItem::separator(),
        ]);
    }
    app_items.push(MenuItem::action("Quit Roboco", Quit));

    let mut menus = vec![
        Menu::new("Roboco").items(app_items),
...
// (`cargo run`) show "Roboco" and the crate version instead of the process name.
#[cfg(target_os = "macos")]
mod about_panel {
    ...
    pub(super) fn show() {
        ...
        let name = NSString::from_str("Roboco");
