...
/// Gap between markdown blocks inside one message (roboco mdBlockGap).
...
// Table metrics — a port of mugen-markdown 0.6.2's `TableBlock` under roboco's
...
/// Uniform cell padding in px (roboco `table.cellPadding`).
pub const TABLE_CELL_PADDING: f32 = 12.0;
/// Hairline between rows in px (roboco `table.gap`).
pub const TABLE_DIVIDER: f32 = 1.0;
/// Header row font weight (roboco `table.headerWeight` = 700).
...
/// Minimum rendered column width in px, padding included (roboco
...
/// Hairline tone (roboco md theme `table.borderColor`: rgba(255,255,255,0.1)).
...
/// Tight monochrome heading scale (roboco: h2 ≈ 16px semibold; headings step
...
/// A GFM table — a port of mugen-markdown's `TableBlock` under roboco's md
...
/// roboco's `table.headerWeight`; strong runs never drop below semibold).
fn flatten_runs_weighted(runs: &[InlineRun], theme: &Theme, base_weight: FontWeight) -> FlatText {
    ...
    for run in runs {
        ...
        // Links stay monochrome — foreground with an underline (roboco's md
