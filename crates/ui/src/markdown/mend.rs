...
//! `remend`, ported to roboco's incremental parser).
//!
...
//! handlers don't apply here — roboco renders raw HTML literally and has no
//! math):
...
pub const PENDING_LINK_URL: &str = "roboco:pending-link";
...
mod tests {
    ...
    fn links_mend_to_pending_sentinel() {
        mends("[docs](https://x.dev/lo", "[docs](roboco:pending-link)");
        mends("[docs](", "[docs](roboco:pending-link)");
        mends("see [do", "see [do](roboco:pending-link)");
        mends("![alt](https://x/i.p", "![alt](roboco:pending-link)");
        stays("see [");
    ...
    fn link_urls_allow_nested_parens() {
        ...
        mends("[a](https://x.dev/(y", "[a](roboco:pending-link)");
    }

    #[test]
    fn emphasis_inside_link_text_closes_inside() {
        mends("[**a", "[**a**](roboco:pending-link)");
        mends("**a [b", "**a [b](roboco:pending-link)**");
    }
