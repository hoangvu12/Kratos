...
//!
//! The set mirrors the original roboco's icon usage exactly:
...
//!   are ports of the hand-drawn inline SVGs in roboco's `terminal-panel.tsx` /
//!   `composer-actions.tsx`.
//! - The harness brand marks (`claude-mark`, `openai-mark`, `cursor-mark`) are
//!   ports of roboco's `icons.tsx`. gpui tints SVGs with the text color, so the
...
icon_assets![
    ...
    // Mirrored variant (roboco window-controls.tsx `-scale-x-100`): the LEFT
    ...
    // Hand-drawn roboco glyphs (terminal-panel.tsx / composer-actions.tsx /
...
/// The Claude mark's brand orange (`#D97757`) — roboco keeps it even on the
/// monochrome surface.
