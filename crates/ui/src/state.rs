...
//! port, mirroring roboco: if an engine is listening it connects over WebSocket
...
pub struct EngineBootConfig {
    /// Data directory for the embedded engine (`~/.roboco`).
...
// grouping, the boot gate, relative times) live in `roboco_proto::view`, pure
...
pub struct AppState {
    ...
    pub connectivity: roboco_proto::Connectivity,
...
impl AppState {
    pub fn new() -> Self {
        Self {
            ...
            connectivity: roboco_proto::Connectivity::default(),
    ...
    pub fn apply_connectivity(&mut self, connectivity: roboco_proto::Connectivity) {
    ...
    pub fn apply_transfers(&mut self, transfers: Vec<roboco_proto::TransferProgress>) {
    ...
    pub fn open_deep_link(&mut self, url: &str, cx: &mut Context<Self>) {
        match crate::links::parse_roboco_conversation_link(url) {
            Ok(link) => {
...
pub use roboco_proto::version_triple;
...
fn spawn_queue_watch(
    ...
    struct QueueFrame {
        #[serde(default)]
        items: Vec<roboco_doc::QueuedMessage>,
    }
    cx.spawn(async move |this, cx| {
        const RETRY_DELAY: std::time::Duration = std::time::Duration::from_secs(2);
        'resubscribe: loop {
...
mod tests {
    ...
    async fn remote_viewport_treats_legacy_daemon_as_ready() {
        ...
        let server = tokio::spawn(roboco_rpc::serve_ws_listener(
            listener,
    ...
    async fn bootstrap_reports_local_assembly_failure_before_returning_a_handle() {
        let dir = tempfile::tempdir().unwrap();
        roboco_engine::EngineProfile::local(dir.path()).unwrap();
    ...
    async fn bootstrap_connects_when_daemon_is_listening() {
        ...
        let listener = tokio::net::TcpListener::bind("127.0.0.1:0").await.unwrap();
        let port = listener.local_addr().unwrap().port();
        tokio::spawn(roboco_rpc::serve_ws_listener(listener, core.rpc_service()));
    ...
    fn transfer_percent_tracks_snapshots_by_upload_id() {
        ...
        let frame = |id: &str, done, total| roboco_proto::TransferProgress {
            upload_id: id.into(),
    ...
    fn project_labels_from_cwd() {
        assert_eq!(project_label(Some("/home/w/dev/roboco")), "roboco");
        assert_eq!(project_label(Some("/home/w/dev/roboco/")), "roboco");
    ...
    fn grouped_sidebar_preserves_recency_order() {
        ...
        let chats = [
            chat_with_cwd("a", 9, Some("/dev/roboco")),
            chat_with_cwd("b", 8, Some("/dev/zed")),
            chat_with_cwd("c", 7, Some("/dev/roboco")),
        ...
        assert_eq!(labels, ["roboco", "zed", "No project"]);
        let roboco_ids: Vec<&str> = groups[0].chats.iter().map(|c| c.id.as_str()).collect();
        assert_eq!(roboco_ids, ["a", "c"]);
        assert!(group_chats(std::iter::empty()).is_empty());
    }

    #[test]
    fn relative_times_match_roboco_format() {
        let now = Utc::now();
    ...
    fn chat_location_joins_project_and_branch() {
        ...
        c.branch = Some("roboco/rebalance".into());
        assert_eq!(
            chat_location(&c).as_deref(),
            Some("soccertcg · roboco/rebalance")
        );
        c.branch = None;
    ...
    fn explicit_capabilities_distinguish_same_version_builds() {
        ...
        assert!(state.device_supports("personal", roboco_proto::capabilities::MESSAGE_QUEUE_V1));
        assert!(!state.device_supports("upstream", roboco_proto::capabilities::MESSAGE_QUEUE_V1));
    }

    #[test]
    fn delivery_degradation_and_queued_sends_tell_the_truth() {
        use roboco_proto::{ChatConnectivity, ConnectivityState};
        let now = Utc::now();
        let mut s = AppState::default();
