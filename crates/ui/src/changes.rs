...
use roboco_proto::{Chat, CheckoutDiff, GitHistoryCommit};
...
pub struct DiffHighlights {
    pub old: Option<Arc<roboco_syntax::HighlightedDocument>>,
    pub new: Option<Arc<roboco_syntax::HighlightedDocument>>,
}
...
impl DiffHighlights {
    ...
    pub fn spans(&self, line: &DiffLine) -> &[roboco_syntax::HighlightSpan] {
...
fn excerpt_side(
    ...
    for hunk in &file.hunks {
        ...
        let source = visible
            ...
            .collect::<Vec<_>>()
            .join("\n");
        let document = roboco_syntax::highlight(roboco_syntax::HighlightRequest {
            source: &source,
            path: Some(path),
            fence_tag: None,
        })
    ...
    Some(Arc::new(roboco_syntax::HighlightedDocument {
        language,
        lines,
    }))
}
...
fn excerpt_highlights(file: &FileDiff, language: Lang) -> Option<DiffHighlights> {
    if !roboco_syntax::supports_language(language) {
        return None;
    }
...
fn sources_match_patch(file: &FileDiff, response: &roboco_proto::CheckoutFileDiffText) -> bool {
    let old = response
        .old_text
        .as_deref()
...
fn full_highlights(
    file: &FileDiff,
    language: Lang,
    response: &roboco_proto::CheckoutFileDiffText,
    ...
    if old.is_none() && new.is_none() && roboco_syntax::supports_language(language) {
        return None;
    }
    Some(DiffHighlights { old, new })
}
...
impl Changes {
    ...
    fn request_highlight(
        ...
        if !roboco_syntax::supports_language(lang) {
        ...
        let fetch_task = match (active, engine) {
            (Some(diff), Some(engine)) => Some(cx.spawn(async move |this, cx| {
                let request = roboco_proto::GetCheckoutFileDiffTextRequest {
                ...
                let response = engine
                    ...
                    .ok()
                    .and_then(|value| {
                        serde_json::from_value::<roboco_proto::CheckoutFileDiffText>(value).ok()
                    });
    ...
    fn render_file_header(
        ...
        // Chevron (roboco checkout-diff-sidebar): chevron-right closed,
...
fn diff_line_row(
    line: &DiffLine,
    spans: &[roboco_syntax::HighlightSpan],
    theme: &Theme,
    gutter_px: f32,
...
/// column, and paint-only syntax runs (roboco checkout-diff-sidebar).
...
";
    ...
    fn sticky_header_uses_the_content_theme_in_dark_and_light() {
        use roboco_theme::{AccentSelection, SurfacePreference};
    ...
    fn phases() {
        ...
        summarized.files.push(roboco_proto::DiffFileSummary {
            path: "x".into(),
            old_path: None,
    ...
    fn full_diff_highlights_map_old_new_and_context_by_source_line() {
        ...
        assert!(
            highlights
                .spans(&deleted)
                .iter()
                .any(|span| span.kind == roboco_syntax::HighlightKind::Function)
        );
        assert!(
            highlights
                .spans(&added)
    ...
    fn split_line_runs_use_affected_old_and_new_documents() {
        ...
        for (path, source, required) in [
            (
                ...
                roboco_syntax::HighlightKind::Tag,
            ),
            (
                "src/Greeter.kt",
                "fun greet(name: String) = println(name)",
                roboco_syntax::HighlightKind::Function,
            ),
            (
                "Dockerfile",
                "RUN echo \"hello\"",
    ...
    fn excerpt_parses_old_and_new_hunks_as_separate_documents() {
        ...
        assert!(
            highlights
                .spans(deleted)
                .iter()
                .any(|span| span.kind == roboco_syntax::HighlightKind::Comment)
        );
        assert!(
            highlights
                .spans(added)
    ...
    fn mismatched_full_sources_are_rejected_atomically() {
        ...
        let response = roboco_proto::CheckoutFileDiffText {
