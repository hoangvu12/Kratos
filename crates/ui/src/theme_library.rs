...
use gpui::{App, Global};
use roboco_theme::vscode::SourceCompilation;
use roboco_theme::{
...
fn reconcile_and_refresh(cx: &mut App) {
    ...
    for appearance_kind in [Appearance::Light, Appearance::Dark] {
        let model = if appearance_kind.is_light() {
            roboco_theme::Appearance::Light
        } else {
            roboco_theme::Appearance::Dark
        };
...
mod tests {
    ...
    fn failed_persistence_leaves_library_state_unchanged() {
        ...
        next.entries.push(CustomThemeEntry {
            ...
            source: roboco_theme::CustomThemeSource::ImportedSnapshot {
                imported_from: None,
            },
            family: roboco_theme::ThemeFamily {
                id: "unpersisted".into(),
                name: "Unpersisted".into(),
                variants: Vec::new(),
            },
            ...
            status: roboco_theme::CustomThemeStatus::Ready,
        });
