...
impl ArchivedPage {
    ...
    fn unarchive(&mut self, chat_id: String, cx: &mut Context<Self>) {
        let Some(engine) = self.state.read(cx).target_for_id(&chat_id).ok() else {
            return;
        };
...
impl Render for ArchivedPage {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let items: Vec<AnyElement> = rows
            ...
            .map(|(ix, chat)| {
                let title: SharedString = chat
                    ...
                    .into();
                // Unknown device → no fragment at all (roboco renders the
                ...
                // roboco settings.archived.tsx row: archive tile, medium title
                ...
                div()
                    ...
                    .child(
                        div()
                            ...
                            .child({
                                ...
                                // own tone (roboco: a plain span inheriting
                    ...
                    .child(
                        // Hidden until the row is hovered (roboco `opacity-0
        ...
        let body: AnyElement = if items.is_empty() {
            // Centered empty state (roboco settings.archived.tsx).
            div()
                .mt(px(96.0))
                .flex()
                .flex_col()
                ...
                .child(
                    ...
                    // effectively ~20% glyph (roboco settings.archived.tsx).
                    crate::icons::icon(crate::icons::ARCHIVE_MINIMALISTIC)
                        .size(px(28.0))
