...
//! (roboco parity: localStorage `roboco.composer.defaults:v1`, defaults.ts).
//!
...
use roboco_proto::{HarnessId, ReasoningLevel};
...
/// Remembered model per harness — id plus display label, mirroring roboco's
...
pub struct ComposerDefaults {
    ...
    /// Last reasoning level picked (global, like roboco's `reasoning` key).
...
impl ComposerDefaults {
    ...
    /// Remember a pick (roboco `saveDefaults({ harness, modelByHarness })`).
