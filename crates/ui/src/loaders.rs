//! Loaders: the roboco pulse loader, the gradient matrix spinner, and the boot
...
use crate::motion::{self, GRADIENT_SPIN, PULSE_STAGGER, SPLASH_OUT, ROBOCO_PULSE};
use crate::theme::{GlyphPalette, Theme};

// Shared with the terminal viewport (`roboco_proto::motion`) so both animate the
// same loaders from the same numbers.
pub use roboco_proto::motion::{
    MARK_CELLS, MARK_SPREAD, MATRIX_SIDE, ROBOCO_CELLS, mark_cell_stagger,
};

/// The animated roboco mark (roboco-loader.tsx `RobocoLoader`): the full logo
...
pub fn roboco_mark_loader(
    ...
    let delta = motion::pulse_delta(&ROBOCO_PULSE, view, cx);
    div()
        .relative()
        .w(px(820.0 * scale))
        .h(px(height_px))
        .children(MARK_CELLS.iter().map(move |&(x, y)| {
            ...
            div()
                ...
                .child({
                    ...
                    // the stagger ADDS phase (roboco-loader.tsx delayFor).
...
pub fn roboco_loader(
    ...
    div()
        ...
        .items_center()
        .gap(px(slot / 2.0))
        .children((0..ROBOCO_CELLS).map(move |i| {
...
pub use roboco_proto::motion::{GSPIN_DIM, GSPIN_ROW_TINTS};

/// The gradient matrix spinner (WorkingIndicator), ported from roboco's
    ...
    let content = div()
        ...
        .child(
            div()
                ...
                .child(SharedString::from("Setting up Roboco environment")),
        );
    if fading {
...
const _: () = {
    ...
    assert!(ROBOCO_PULSE.duration_ms == 2400);
