...
fn main() -> anyhow::Result<()> {
    ...
    let output = PathBuf::from(
        std::env::args()
            .nth(1)
            .unwrap_or_else(|| "/tmp/roboco-browser-captures".into()),
    );
    ...
    gpui_platform::application().with_assets(icons::Assets).run(move |cx| {
        ...
        cx.spawn(async move |cx| {
            let run: anyhow::Result<()> = async {
                pause(cx, 1200).await;
                if std::env::var_os("ROBOCO_TRANSCRIPT_LINK_FIXTURE_ONLY").is_some() {
                ...
                state.update(cx, |s, cx| {
                    ...
                    s.receive_transcript_frame(roboco_doc::TranscriptFrame::Reset { reset: entries }, cx).unwrap();
                });
                ...
                #[cfg(target_os = "macos")]
                {
                    ...
                    cx.update(|cx|appearance::set_surface(roboco_theme::SurfacePreference::Opaque,cx));
                    pause(cx,500).await;
                    ...
                    cx.update(|cx| {appearance::set_mode(appearance::AppearanceMode::Dark,cx);appearance::set_surface(roboco_theme::SurfacePreference::Frosted,cx);});
