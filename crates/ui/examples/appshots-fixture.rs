...
fn main() -> anyhow::Result<()> {
    ...
    let core = runtime.block_on(async {
        roboco_engine::EngineCore::assemble(
    ...
    let ipc_port = port();
    let _ipc = runtime.block_on(roboco_engine::serve_ipc(ipc_port, core.rpc_service()))?;
    ...
    gpui_platform::application().with_assets(icons::Assets).run(move |cx| {
        ...
        cx.spawn(async move |cx| {
            let run:anyhow::Result<()>=async {
                pause(cx,1000).await;
                state.update(cx,|s,cx| {s.receive_transcript_frame(roboco_doc::TranscriptFrame::Reset {reset:serde_json::from_value(serde_json::json!([
                ...
                state.update(cx,|s,cx|{s.queue.clear();s.receive_transcript_frame(roboco_doc::TranscriptFrame::Reset{reset:serde_json::from_value(serde_json::json!([
