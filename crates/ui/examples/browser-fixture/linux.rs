...
use roboco_ui::browser::BrowserSurface;
...
pub(super) fn dispatch(
    ...
    if std::env::var_os("ROBOCO_BROWSER_NATIVE_POINTER").is_some() {
        ...
        if let Some((position, action)) = pointer {
            let id = if let Ok(id) = std::env::var("ROBOCO_BROWSER_CAPTURE_WINDOW") {
                id
            } else {
...
pub async fn exercise(
    ...
    if let Ok(id) = std::env::var("ROBOCO_BROWSER_CAPTURE_WINDOW") {
        let scale = AnyWindowHandle::from(window).update(cx, |_, w, _| w.scale_factor())?;
    ...
    cx.update(|cx| {
        ...
        appearance::set_surface(roboco_theme::SurfacePreference::Frosted, cx);
    });
    pause(cx, 300).await;
