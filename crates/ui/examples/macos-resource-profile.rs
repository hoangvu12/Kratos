...
struct Frame {
    at: u64,
    frame: roboco_doc::TranscriptFrame,
}
...
fn main() -> anyhow::Result<()> {
    ...
    let app = gpui::Application::with_platform(platform).with_assets(icons::Assets)
        .run_embedded(move |cx| {
            ...
            composer::init(cx, roboco_ui::settings::ComposerSendBehavior::default());
            terminal::panel::init(cx);
            app_menus::init(cx);
            let state = cx.new(|_| {
                let mut state = state::AppState::new();
                state.connection = roboco_proto::view::ConnectionStatus::Ready;
                ...
                let background_chats = std::env::var("ROBOCO_PROFILE_BACKGROUND_CHATS")
                ...
                if std::env::var_os("ROBOCO_VERIFY_SIDEBAR_ROWS").is_some() {
                    ...
                    for chat in &mut state.chats {
                        chat.source_context = Some(roboco_proto::ConversationSourceContext {
    ...
    loop {
        objc::rc::autoreleasepool(|| {
            ...
            while frames.peek().is_some_and(|f| f.at - first_at <= elapsed) {
                ...
                let text_only = matches!(&frame.frame, roboco_doc::TranscriptFrame::Delta {
                ...
                app.update(|cx| {
                    state.update(cx, |state, cx| {
                        ...
                        let streaming = state
                            .transcript
                            .iter()
                            .any(|entry| entry.status == Some(roboco_doc::MessageStatus::Streaming));
                        state.apply_sessions(vec![roboco_proto::Session {
                            ...
                            status: if streaming {
                                ...
                                roboco_proto::SessionStatus::Idle
                            },
    ...
    if std::env::var_os("ROBOCO_VERIFY_CACHE").is_some() {
        ...
        if std::env::var_os("ROBOCO_VERIFY_INTERACTIONS").is_some() {
