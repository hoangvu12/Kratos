...
//! [`default_stall_bound`] (`ROBOCO_OPENCODE_STALL_MS`, 0 disables) errors
...
const STARTUP_TIMEOUT_ENV: &str = "ROBOCO_OPENCODE_STARTUP_TIMEOUT_SECS";
...
const STALL_ENV: &str = "ROBOCO_OPENCODE_STALL_MS";
        ...
        if let Some(stderr) = child.stderr.take() {
            ...
            tokio::spawn(async move {
                ...
                while let Ok(Some(line)) = lines.next_line().await {
                    tracing::debug!(target: "roboco_harness::opencode", "stderr: {line}");
                    tail.push(&line);
                }
            });
        }
