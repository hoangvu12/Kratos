...
fn grok_spec() -> AcpAgentSpec {
    AcpAgentSpec {
        ...
        // (the user's TUI) reads as total silent non-response in roboco.
        ...
        stall_hint: "The agent process is likely wedged — a stale shared leader \
             process or a hung startup check; roboco launches it with --no-leader \
...
/// Map an advertised `thought_level` value id onto roboco's ladder.
...
/// roboco's own (forced to the no-prompts choice), model rides the model rows,
...
/// catalogs (roboco never declares the boolean config capability, so adapters
...
fn initialize_params(harness: HarnessId) -> Value {
    ...
    if harness == HarnessId::Devin {
        ...
        // separate subagentControl extension: Roboco has no matching UI yet.
    ...
    json!({
        ...
        // is what roboco wants — the working tree is the source of truth for
...
/// bypassPermissions and the codex harness's approvalPolicy "never" (roboco
...
fn handle_server_request(
    ...
    match method {
        ...
        _ => {
            tracing::debug!(target: "roboco_harness::acp", "unhandled server request: {method}");
...
async fn run_session(session: Session) {
        ...
        res = tokio::time::timeout(handshake_timeout, setup) => {
            ...
            match res {
                ...
                Err(e) => {
                    ...
                    tracing::warn!(target: "roboco_harness::acp", %error, "agent setup failed");
                    let _ = event_tx
    ...
    // ROBOCO_ACP_QUIET_SETTLE_MS is intentionally no longer honored (#296).
        ...
        tokio::select! {
            res = async { turn.as_mut().expect("guarded by if").await }, if turn.is_some() => {
                ...
                if let Some(text) = queued_steers.pop_front() {
                    ...
                    prompt_seq += 1;
                    current_prompt_id =
                        prompt_complete_extension.then(|| format!("roboco-p{prompt_seq}"));
