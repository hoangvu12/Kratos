...
//! ONCE into a roboco-owned prefix (`~/.roboco/adapters/<pkg>/<version>` on
...
const OK_MARKER: &str = ".roboco-install-ok";
const INSTALL_TIMEOUT: Duration = Duration::from_secs(600);

/// Managed adapter storage. `$ROBOCO_ADAPTERS_DIR` wins, followed by
/// `$ROBOCO_DATA_DIR/adapters`. Windows defaults to
/// `%LOCALAPPDATA%/Roboco/adapters` (or `%USERPROFILE%/AppData/Local/...`);
/// Unix keeps `~/.roboco/adapters`.
...
fn adapters_root_with(
    ...
    if let Some(dir) = value("ROBOCO_ADAPTERS_DIR") {
        return Some(dir);
    }
    if let Some(dir) = value("ROBOCO_DATA_DIR") {
        return Some(dir.join("adapters"));
    }
    if platform == crate::executable::Platform::Windows {
        value("LOCALAPPDATA")
            .map(|dir| dir.join("Roboco").join("adapters"))
            .or_else(|| {
        ...
        value("HOME").map(|home| home.join(".roboco").join("adapters"))
    }
}
...
/// A roboco-owned shim script materialized INSIDE a managed install dir, for
...
/// roboco-owned shim rather than through a bin entry. Installs the pin once,
...
mod tests {
    ...
    fn adapters_root_has_injected_windows_precedence_and_fallbacks() {
        let explicit = PathBuf::from(r"D:\Roboco adapters");
        let data = PathBuf::from(r"E:\Roboco data");
        let local = PathBuf::from(r"C:\Users\Ada\AppData\Local");
        let profile = PathBuf::from(r"C:\Users\Ada");
        assert_eq!(
            adapters_root_with(
                &env(&[
                    ("ROBOCO_ADAPTERS_DIR", explicit.clone().into_os_string()),
                    ("ROBOCO_DATA_DIR", data.clone().into_os_string()),
        ...
        assert_eq!(
            adapters_root_with(
                &env(&[("ROBOCO_DATA_DIR", data.clone().into_os_string())]),
                crate::executable::Platform::Windows,
            ),
        ...
        assert_eq!(
            ...
            Some(local.join("Roboco").join("adapters"))
        );
        assert_eq!(
            adapters_root_with(
