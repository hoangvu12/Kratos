// roboco cursor shim — a thin, roboco-owned wrapper around the PINNED
...
// every bump). Materialized by roboco into the SDK's managed install dir so
...
// Roboco runs concurrent shims on the same cwd as a matter of course — the
...
const STATE_BASE =
  process.env.ROBOCO_CURSOR_STATE_DIR || path.join(os.homedir(), ".roboco", "cursor-state");
