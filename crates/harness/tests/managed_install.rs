...
//! `$ROBOCO_ADAPTERS_DIR`, spawn it via node, and reach SessionStarted (the
...
//! `cargo test -p roboco-harness --test managed_install -- --ignored`.
//!
//! Single-test binary: it mutates ROBOCO_ADAPTERS_DIR process-wide.
...
async fn managed_install_reaches_session_started() {
    ...
    unsafe {
        std::env::set_var("ROBOCO_ADAPTERS_DIR", adapters.path());
    ...
    let version_dir = std::fs::read_dir(&root)
        ...
        .path();
    assert!(version_dir.join(".roboco-install-ok").exists());
}
