//! Roboco's source-neutral theme domain model.
//!
...
impl Default for ThemeSelection {
    fn default() -> Self {
        Self {
            light: "roboco-light".into(),
            dark: "roboco-dark".into(),
        }
    }
}
...
impl ThemeRegistry {
    ...
    pub fn resolve(&self, selection: &ThemeSelection, appearance: Appearance) -> &ThemeVariant {
        self.variant(selection.variant_id(appearance))
            ...
            .expect("the built-in registry always contains both Roboco variants")
    }
...
mod tests {
    ...
    fn builtins_have_complete_provenance_and_no_validation_errors() {
        ...
        assert!(registry.variant("roboco-light").is_some());
        assert!(registry.variant("roboco-dark").is_some());
