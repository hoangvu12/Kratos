...
///
/// Semantics from roboco `foldEventIntoParts`:
        ...
        AgentEvent::Subagent {
            ...
            let status = match event.as_ref() {
                AgentEvent::Done { status, .. } => Some(match status {
                    roboco_proto::DoneStatus::Errored => SubagentStatus::Failed,
...
/// continuations (`continuation_id(root, i)`), matching `splitMessageEntry` in roboco.
    ...
    fn session_started_resets_accumulator() {
        ...
        fold_event_into_parts(
            ...
            &AgentEvent::SessionStarted {
                harness: roboco_proto::HarnessId::Mock,
                model: "m".into(),
                tools: vec![],
                cwd: "/".into(),
                session_id: "s".into(),
