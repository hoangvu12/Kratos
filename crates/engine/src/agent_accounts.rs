...
//! (feature-inventory §3.7 "Agent accounts"; port of roboco's `agent-accounts.ts`).
//!
...
//!   whole-account session tokens, which roboco never reads.
//!
...
//! `GetCurrentPeriodUsage` call the Cursor app itself makes). Unlike roboco (fetch on every
...
/// One saved login (`{slotId}.json`), same field surface as roboco's slot files.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
struct Slot {
    id: String,
...
enum LoginFlow {
    ...
    Spawned {
        ...
        child: Arc<Mutex<Option<roboco_harness::process::Child>>>,
...
impl AgentAccounts {
    ...
    async fn start_codex_login(&self) -> Result<AgentLoginStart, EngineError> {
        ...
        let mut command = match roboco_harness::codex::login_command(&home) {
            Ok(command) => command,
            Err(err) => {
                let _ = std::fs::remove_dir_all(&home);
                return Err(EngineError::Other(match err {
                    roboco_harness::HarnessError::NotInstalled(hint) => {
                        format!(
    ...
    async fn start_cursor_login(&self) -> Result<AgentLoginStart, EngineError> {
        ...
        let home = self
            ...
            .join(format!(".login-{login_id}"));
        std::fs::create_dir_all(&home)?;
        let mut cmd = roboco_harness::cursor::login_command(&home.join("auth.json"))
            .await
            .map_err(|e| {
                let _ = std::fs::remove_dir_all(&home);
                EngineError::Other(format!("Could not start the Cursor login: {e}"))
            })?;
        cmd.stdin(roboco_harness::process::Stdio::null())
            .stdout(roboco_harness::process::Stdio::piped())
            .stderr(roboco_harness::process::Stdio::piped());
    ...
    /// Lazy TTL sweep (roboco uses a background fiber; native reaps on the next
...
type LoginChildHandles = (
    Arc<Mutex<Option<roboco_harness::process::Child>>>,
    Arc<Mutex<String>>,
...
/// access.
fn wire_login_child(mut child: roboco_harness::process::Child) -> LoginChildHandles {
