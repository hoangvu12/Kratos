...
struct RelayCommandParams {
    ...
    entry: roboco_doc::SessionCommandEntry,
}
...
enum MutateParams {
    ...
    /// Full-config replace on the chat row (roboco `SetChatConfig`): the
...
impl EngineRpc {
    ...
    async fn file_search_root(&self, p: &FileSearchParams) -> Result<std::path::PathBuf, RpcError> {
        let target = roboco_proto::WorkspaceTarget {
...
/// The transcript watch as delta frames (`roboco_doc::transcript_delta`): a
...
fn doc_messages_stream(
    rx: watch::Receiver<std::sync::Arc<Vec<roboco_doc::SessionMessageEntry>>>,
    doc: std::sync::Arc<roboco_doc::SessionDoc>,
) -> BoxStream<'static, serde_json::Value> {
    use roboco_doc::transcript_delta::{TranscriptFrame, diff_transcript};
    futures::stream::unfold(
        (
            rx,
            None::<std::sync::Arc<Vec<roboco_doc::SessionMessageEntry>>>,
            doc,
            None,
        ),
    ...
    async fn handle(&self, method: &str, params: serde_json::Value) -> Result<RpcReply, RpcError> {
        ...
        match method {
            ...
            methods::WATCH_PREVIEWS => {
                let p: roboco_proto::WatchPreviewsParams = parse_params(params)?;
                if self
                    .workspace
                    .chat(&p.chat_id)
                    .map_err(|e| RpcError::Failed(e.to_string()))?
            ...
            methods::GET_CHECKOUT_DIFF => {
                ...
                Box::pin(async move {
                    ...
                    .map_err(|e| RpcError::Failed(e.to_string()))?;
                    RpcReply::value(&roboco_proto::CheckoutDiff {
            ...
            methods::GET_CHECKOUT_FILE_DIFF_TEXT => {
                ...
                Box::pin(async move {
                    let p: roboco_proto::GetCheckoutFileDiffTextRequest = parse_params(params)?;
                    let identity =
                    ...
                    let stale = || roboco_proto::CheckoutFileDiffText {
                    ...
                    RpcReply::value(&roboco_proto::CheckoutFileDiffText {
            ...
            methods::LIST_DRIVES => {
                let drives = self
                    ...
                    .await
                    .map_err(|e| RpcError::Failed(e.to_string()))?;
                RpcReply::value(&roboco_proto::DriveListing { drives })
            }
            ...
            methods::LIST_WORKSPACE_DIRECTORY => {
                let request: roboco_proto::ListWorkspaceDirectoryRequest = parse_params(params)?;
            ...
            methods::SEARCH_WORKSPACE_FILES => {
                let request: roboco_proto::SearchWorkspaceFilesRequest = parse_params(params)?;
            ...
            methods::READ_WORKSPACE_IMAGE => {
                let request: roboco_proto::ReadWorkspaceImageRequest = parse_params(params)?;
            ...
            methods::READ_WORKSPACE_FILE => {
                let request: roboco_proto::ReadWorkspaceFileRequest = parse_params(params)?;
            ...
            methods::WATCH_WORKSPACE_FILES => {
                let request: roboco_proto::WatchWorkspaceFilesRequest = parse_params(params)?;
...
mod context_usage_tests {
    ...
    async fn context_only_commits_reach_remote_watch_and_reconnect() {
        let host = roboco_doc::SessionDoc::init("context-chat").unwrap();
        host.update_context_usage(Some(42000), Some(200000))
            .unwrap();
        // The viewing engine reads a replicated document, with no harness process.
        let remote = Arc::new(roboco_doc::SessionDoc::from_doc(loro::LoroDoc::new()));
        remote
            .doc()
