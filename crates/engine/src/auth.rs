...
//! ARCHITECTURE §5). Port of roboco's `apps/backend/src/auth.ts`.
//!
...
/// SignedIn{user, orgId?}`). Serializes as the canonical [`roboco_proto::AuthState`]
...
impl AuthState {
    ...
    pub fn to_proto(&self) -> roboco_proto::AuthState {
        let profile = |user: &AuthUser| roboco_proto::UserProfile {
            id: user.id.clone(),
            email: user.email.clone(),
            name: user.name.clone(),
        };
        match self {
            AuthState::SignedOut => roboco_proto::AuthState::SignedOut,
            AuthState::NeedsOrganization { user } => roboco_proto::AuthState::NeedsOrganization {
                user: profile(user),
            },
            AuthState::SignedIn { user, org_id } => roboco_proto::AuthState::SignedIn {
                user: profile(user),
                org_id: org_id.clone(),
            },
        }
...
pub struct AuthConfig {
    ...
    /// Dev-mode bearer/user id (mirrors the old `ROBOCO_EDGE_TOKEN` behavior).
...
impl Auth {
    ...
    fn finish_sign_in(&self, result: SignInResult, generation: u64) -> Result<(), EngineError> {
        ...
        if sign_in.generation != generation {
            return Err(EngineError::Other(
                "sign-in was canceled — start again from Roboco".into(),
            ));
        }
...
/// The relay/room token seam: `Auth` IS a [`roboco_rpc::TokenSource`], so the host relay
/// and link cache always dial with a fresh bearer after refreshes.
#[async_trait::async_trait]
impl roboco_rpc::TokenSource for Auth {
...
async fn handle_loopback_conn(
    ...
    let (status, body) = if path != "/callback" {
        ...
        let invalid_callback = || {
            (
                "400 Bad Request",
                page("Invalid or expired sign-in link. Start again from Roboco."),
            )
        };
        match (code, state) {
            (Some(code), Some(state)) => match auth.take_pending(state) {
                Some(generation) => match auth.exchange_code(code).await {
                    Ok(result) => match auth.finish_sign_in(result, generation) {
                        Ok(()) => (
                            "200 OK",
                            page("Signed in. You can close this tab and return to Roboco."),
                        ),
                        Err(err) => {
                            ...
                            (
                                "409 Conflict",
                                page(
                                    "This sign-in was canceled. Start again from Roboco if you still want to enable sync.",
                                ),
                            )
                        }
                    },
                    Err(err) => {
                        ...
                        (
                            "502 Bad Gateway",
                            page("Sign-in failed during token exchange — check the Roboco logs."),
                        )
                    }
                },
...
mod tests {
    ...
    fn auth_state_serializes_as_proto_shape() {
        ...
        let parsed: roboco_proto::AuthState = serde_json::from_value(value).expect("proto parse");
        assert!(matches!(parsed, roboco_proto::AuthState::SignedIn { .. }));
        assert_eq!(
