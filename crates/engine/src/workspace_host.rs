//! Engine-local device, space, chat, and session registry with persisted snapshots.
...
/// Debounce local registry snapshots.
...
impl WorkspaceHost {
    ...
    pub fn open(store: Arc<DocsStore>, config: WorkspaceHostConfig) -> Result<Self, EngineError> {
        ...
        state.sessions.retain(|s| s.device_id == config.device_id);
        state.spaces.retain(|s| s.device_id == config.device_id);
    ...
    pub fn read_chats(&self) -> Result<Vec<Chat>, EngineError> {
        Ok(self
            .read(|doc| doc.read_chats())?
            .into_iter()
            .filter(|row| row.device_id == self.inner.config.device_id)
            .collect())
    }
    ...
    pub fn read_devices(&self) -> Result<Vec<Device>, EngineError> {
        Ok(self
            ...
            .into_iter()
            .filter(|row| row.id == self.inner.config.device_id)
            .collect())
    }

    pub fn read_sessions(&self) -> Result<Vec<Session>, EngineError> {
        Ok(self
            .read(|doc| doc.read_sessions())?
            .into_iter()
            .filter(|row| row.device_id == self.inner.config.device_id)
            .collect())
    }
...
impl WorkspaceHostInner {
    ...
    fn publish_lists(&self, clock_tick: bool) {
        match lock(&self.reg).read_all() {
            Ok(mut state) => {
                state.devices.retain(|d| d.id == self.config.device_id);
                state.chats.retain(|c| c.device_id == self.config.device_id);
                state
                    .sessions
                    .retain(|s| s.device_id == self.config.device_id);
                state
                    .spaces
                    .retain(|s| s.device_id == self.config.device_id);
                if clock_tick {
                    ...
                    let previous = self.devices_tx.borrow();
                    for device in &mut state.devices {
                        if let Some(old) = previous.iter().find(|old| old.id == device.id) {
                            device.last_seen_at = device.last_seen_at.max(old.last_seen_at);
                        }
                    }
                }
    ...
    /// The connected local engine is always present in its own state feed.
