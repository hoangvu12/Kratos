...
use roboco_harness::{CancellationToken, RunControls, SteerMessage};
use roboco_proto::{
...
/// couple of times with a short backoff before falling back (roboco's ladder).
...
impl TitleGenerator {
    ...
    async fn generate(
        ...
        // roboco/<name> branch (guards live inside rename_worktree_branch).
        if let (Some(chat_cwd), Some(branch)) = (&latest.cwd, &latest.branch)
            && branch.starts_with("roboco/")
        {
            match self
                .inner
                .repos
    ...
    async fn run_title_model(
        ...
        let harness_id = settings.harness.or_else(|| {
            if roboco_harness::supports_titles(harness_id) {
                Some(harness_id)
            } else {
                enabled
                    .iter()
                    .copied()
                    .find(|id| roboco_harness::supports_titles(*id))
            }
        })?;
        if !roboco_harness::supports_titles(harness_id) {
            return None;
        ...
        let title_prompt = format!(
            ...
            roboco_harness::TITLE_INSTRUCTIONS,
...
/// The cheapest model a harness offers (roboco's `cheapestModel` heuristic):
...
async fn collect_text(
    harness: &dyn roboco_harness::Harness,
    request: RunRequest,
...
mod tests {
    ...
    async fn tool_use_rejects_the_title_instead_of_accepting_coding_output() {
        let harness = roboco_harness::mock::MockHarness {
            script: vec![
    ...
    impl roboco_harness::Harness for RecordingTitleHarness {
        ...
        fn steering_mode(&self) -> roboco_proto::SteeringMode {
            roboco_proto::SteeringMode::TurnBoundary
        }
        fn reasoning_levels(&self) -> &[ReasoningLevel] {
            &[]
        }
        async fn models(&self) -> Result<Vec<Model>, roboco_harness::HarnessError> {
