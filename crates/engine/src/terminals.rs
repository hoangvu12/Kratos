...
//! roboco's `terminals.ts` over `portable-pty`).
//!
...
//!   owner, so the per-user owner re-checks from roboco's Router land with real
...
use roboco_doc::TERMINAL_OUTPUT_BATCH_MS;
...
mod windows_tests {
    ...
    use base64::Engine as _;
    use base64::engine::general_purpose::STANDARD as BASE64;
    use roboco_proto::TerminalEvent;
