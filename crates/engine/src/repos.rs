...
//! browser (feature-inventory §3.5; port of roboco's `repos.ts` + `folder-lister.ts`).
//!
...
//! created under `~/.roboco/worktrees/<repoName>/<worktreeName>` (NOT the data
//! dir — worktrees are user-facing working checkouts), with an auto-generated name +
//! matching `roboco/<name>` branch. `ROBOCO_WORKTREES_DIR` overrides the root.
//!
...
const NOUNS: &[&str] = &[
    "otter", "harbor", "falcon", "cedar", "meadow", "roboco", "delta", "ember", "lynx", "maple",
...
fn default_worktrees_root() -> PathBuf {
    std::env::var_os("ROBOCO_WORKTREES_DIR")
        .filter(|s| !s.is_empty())
        .map(PathBuf::from)
        .unwrap_or_else(|| home_dir().join(".roboco").join("worktrees"))
}
...
impl Repos {
    ...
    /// remotes, and tags seed the walk, so Roboco's internal refs never leak
    ...
    /// `{worktrees_root}/<repoName>/<generatedName>`, on a fresh `roboco/<name>`
    /// branch off `branch`.
    pub async fn create_worktree(
        ...
        for attempt in 0..50u64 {
            ...
            if !base.join(&candidate).exists() && !existing.contains(&format!("roboco/{candidate}"))
            {
                name = Some(candidate);
                break;
            }
        ...
        let branch_name = format!("roboco/{name}");
        self.git(
            &[
                "worktree",
                "add",
    ...
    /// Rename a roboco-created worktree branch after its chat's generated title
    /// (port of roboco's `renameWorktreeBranch`). Guards:
    /// - respect an external checkout/rename: only act while the worktree is still
    ///   on `expected_branch` AND that branch is the original `roboco/<folderName>`;
    ...
    pub async fn rename_worktree_branch(
        ...
        let folder = worktree_path
            ...
            .map(|n| n.to_string_lossy().to_string())
            .unwrap_or_default();
        if current != expected_branch || expected_branch != format!("roboco/{folder}") {
            return Ok(current);
        }
    ...
    /// Deletes the worktree's branch ONLY when roboco created it (`roboco/…`) — the
    ...
    /// timeout the thread is simply abandoned (the roboco backend's disposable
...
/// Turn a generated chat title into the semantic portion of a Roboco branch
/// (port of roboco's `worktreeBranchFromTitle`). Roboco NFKD-normalizes accented
...
pub fn worktree_branch_from_title(title: &str) -> String {
    ...
    format!("roboco/{}", if slug.is_empty() { "update" } else { slug })
}
