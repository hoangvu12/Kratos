...
impl HarnessRegistry {
    ...
    pub fn set_title_settings(&self, mut settings: TitleSettings) -> Result<(), String> {
        if let Some(id) = settings.harness {
            if !roboco_harness::supports_titles(id) || !self.enabled_set().contains(&id) {
...
pub fn default_registry() -> HarnessRegistry {
    ...
    registry.register_lazy(
        ...
        Box::new(|| Ok(Arc::new(roboco_harness::ClaudeHarness::new()) as Arc<dyn Harness>)),
    );
    ...
    // roboco_harness::codex::catalog. CLI discovery only happens when a
    ...
    registry.register_lazy(
        ...
        Box::new(|| Ok(Arc::new(roboco_harness::CodexHarness::new()) as Arc<dyn Harness>)),
    );
    ...
    registry.register_lazy(
        ...
        Box::new(|| Ok(Arc::new(roboco_harness::CursorHarness::new()) as Arc<dyn Harness>)),
    );
    ...
    registry.register_lazy(
        ...
        Box::new(|| Ok(Arc::new(roboco_harness::AcpHarness::devin()) as Arc<dyn Harness>)),
    );
    ...
    registry.register_lazy(
        ...
        Box::new(|| Ok(Arc::new(roboco_harness::AcpHarness::grok()) as Arc<dyn Harness>)),
    );
    ...
    registry.register_lazy(
        ...
        Box::new(|| Ok(Arc::new(roboco_harness::OpencodeHarness::new()) as Arc<dyn Harness>)),
    );
    registry
}
...
mod title_tests {
    ...
    fn title_preferences_persist_and_validate_harness_model_pairs() {
        ...
        registry.register(Arc::new(
            roboco_harness::ClaudeHarness::new().with_executable(std::env::current_exe().unwrap()),
        ));
