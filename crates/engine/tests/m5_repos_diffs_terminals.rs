...
async fn repos_round_trip_add_branches_worktrees() {
    ...
    // Worktree add: roboco/<name> branch, isolated dir under the test root.
    let worktree = repos
        .create_worktree(&repo_dir, "main")
        .await
        .expect("worktree");
    assert!(
        worktree.branch.starts_with("roboco/"),
        "branch: {}",
        worktree.branch
    );
    ...
    // worktree's roboco/<name> branch maps to its linked-checkout path, and
    ...
    // Delete: dir removed, roboco branch removed, refs pruned.
    repos
    ...
    assert!(
        ...
        "roboco branch deleted: {branches:?}"
    );
...
async fn diff_file_text_returns_both_checked_sources() {
    ...
    let escape = roboco_proto::DiffFileSummary {
...
async fn diff_capture_truncates_at_patch_cap() {
    ...
    assert!(snapshot.patch.contains("# Roboco diff truncated"));
}
...
async fn rpc_dispatch_for_m5_methods() {
    ...
    unsafe { std::env::set_var("ROBOCO_WORKTREES_DIR", tmp.path().join("worktrees")) };
    let core = assemble(&tmp.path().join("data"));
    let client = roboco_rpc::memory_client(core.rpc_service());
