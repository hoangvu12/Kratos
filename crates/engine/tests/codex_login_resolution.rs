...
use roboco_engine::{AgentAccounts, AgentAccountsConfig};
use roboco_proto::{AgentLoginMode, HarnessId};
...
async fn login_child() {
    ...
    let root = std::env::temp_dir().join(format!("roboco-codex-login-{}", std::process::id()));
