...
use roboco_engine::{EngineCore, HarnessRegistry, RunJournal};
...
fn queue_as_viewer(doc: &SessionDoc, id: &str, payload: SessionCommandPayload) {
    ...
    let based_on =
        doc.read_entries()
            .expect("read entries")
            .last()
            .map(|m| roboco_doc::CommandBasedOn {
...
async fn steer_with_no_live_run_falls_back_to_new_turn() {
    ...
    // dispatch-as-next-turn, per roboco's executor.
    queue_as_viewer(
        handle.doc(),
        "cmd-steer-1",
...
async fn processed_commands_are_skipped_on_redelivery() {
    ...
    let verdict = roboco_doc::evaluate_command(
        entry,
        &roboco_doc::EvaluationContext {
    ...
    assert_eq!(verdict, roboco_doc::CommandDisposition::Skip);
}
...
async fn rpc_surface_over_in_memory_transport() {
    ...
    let harnesses = client
        .call(roboco_rpc::methods::LIST_HARNESSES, serde_json::Value::Null)
        .await
        .unwrap();
    assert_eq!(harnesses[0]["id"], "mock");
    let models = client
        .call(
            roboco_rpc::methods::LIST_MODELS,
    ...
    let mut sessions_stream = client
        .subscribe(roboco_rpc::methods::WATCH_SESSIONS, serde_json::Value::Null)
        .await
        .unwrap();
    ...
    let mut messages_stream = client
        .subscribe(
            roboco_rpc::methods::WATCH_DOC_MESSAGES,
    ...
    let queued = client
        .call(
            roboco_rpc::methods::QUEUE_COMMAND,
    ...
    let settled = loop {
        let item = tokio::time::timeout_at(deadline, messages_stream.recv())
            ...
            .expect("stream alive");
        let frame: roboco_doc::TranscriptFrame = serde_json::from_value(item).unwrap();
        roboco_doc::apply_transcript_frame(&mut materialized, frame).unwrap();
...
async fn respond_input_resolves_pending_question() {
    ...
    impl Harness for AskingHarness {
        ...
        async fn run(
            ...
            let (tx, rx) = tokio::sync::mpsc::channel::<Result<AgentEvent, HarnessError>>(16);
            tokio::spawn(async move {
                let answers = (controls.request_input)(vec![roboco_proto::UserInputQuestion {
                    id: "q1".into(),
                    header: "Pick".into(),
        ...
        async fn run(
            ...
            if request.prompt == "second run" {
                ...
                tokio::spawn(async move {
                    ...
                    let _ = (controls.request_input)(vec![roboco_proto::UserInputQuestion {
                        id: "q1".into(),
                        header: "Pick".into(),
                        question: "Which one?".into(),
                        options: vec!["a".into(), "b".into()],
...
async fn harness_emitted_input_twin_is_dropped_and_answer_resumes() {
    ...
    impl Harness for DoubleEmitHarness {
        ...
        async fn run(
            ...
            let (tx, rx) = tokio::sync::mpsc::channel::<Result<AgentEvent, HarnessError>>(16);
            tokio::spawn(async move {
                let question = roboco_proto::UserInputQuestion {
                    id: "q1".into(),
                    header: "Pick".into(),
                    question: "Which one?".into(),
                    options: vec!["a".into(), "b".into()],
...
async fn attachment_upload_then_run_threads_refs_and_paths() {
    ...
    for (seq, data) in [(0, first), (1, second)] {
        client
            .call(
                roboco_rpc::methods::UPLOAD_CHUNK,
    ...
    let committed = client
        .call(
            roboco_rpc::methods::UPLOAD_COMMIT,
    ...
    // Run with the roboco `withAttachments` transport: refs embedded in the
    ...
    let chunk = client
        .call(
            roboco_rpc::methods::READ_ATTACHMENT_CHUNK,
...
/// Run with: `cargo test -p roboco-engine --test e2e -- --ignored`
#[tokio::test(flavor = "multi_thread")]
        ...
        if routed_dispatch {
            ...
            assert_eq!(
                core.sessions
                    ...
                    .await
                    .unwrap(),
                roboco_engine::sessions::SteerOutcome::Accepted
            );
        }
