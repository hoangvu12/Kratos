...
#
# Everything lives under /tmp/roboco-demo-*; re-runs reuse it. Ctrl-C cleans up.
set -euo pipefail
cd "$(dirname "$0")/.."

DAEMON_DIR=/tmp/roboco-demo-daemon
UI_DIR=/tmp/roboco-demo-ui
IPC=27921
DELAY=""
[[ "${1:-}" == "--slow" ]] && DELAY=350

echo "▸ building (first run takes a few minutes)…"
cargo build -p roboco -q

echo "▸ starting engine daemon on :$IPC"
env ROBOCO_DATA_DIR="$DAEMON_DIR" ROBOCO_IPC_PORT=$IPC ROBOCO_HARNESS=mock \
  ${DELAY:+ROBOCO_MOCK_DELAY_MS=$DELAY} RUST_LOG=warn \
  ./target/debug/roboco headless &
DAEMON_PID=$!
...
probe() { cargo run -q -p roboco-rpc --example rpc_probe -- "ws://127.0.0.1:$IPC" "$@"; }
...
if [[ ! -f "$DAEMON_DIR/.demo-seeded" ]]; then
  ...
  declare -A SPACES=()
  for project in roboco soccertcg roboco aether; do
  ...
  seed "Native Roboco Rust Rewrite"    roboco roboco/main                 0  run
  seed "Rebalance Player Stats Caps"  soccertcg    roboco/rebalance-player-stat-caps  2  run
  seed "Craft Premium TCG Experience" soccertcg    roboco/craft-premium-tcg-exp       26 skip
  seed "Initial Context Exploration"  roboco        roboco/initial-context-exploration 14 skip
  seed "Soccer TCG Repo Creation"     aether       aether/main                       48 skip
  touch "$DAEMON_DIR/.demo-seeded"
fi

echo "▸ opening roboco (composer is live — type into it; --slow shows streaming)"
ROBOCO_DATA_DIR="$UI_DIR" ROBOCO_IPC_PORT=$IPC RUST_LOG=warn ./target/debug/roboco
