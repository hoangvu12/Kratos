...
// one API turn. RSS/CPU for the CLI child are reported separately from Roboco.
...
const profiledBinary = `${output}/roboco-profiled`;
...
const env = { ...process.env, ROBOCO_IPC_PORT: String(port), ROBOCO_DATA_DIR: `${output}/engine`,
  ROBOCO_HARNESS: harness, ROBOCO_FRAME_STATS: process.env.ROBOCO_FRAME_STATS ?? '1', RUST_LOG: 'warn',
  ROBOCO_MOCK_CHARS: '24', ROBOCO_MOCK_DELAY_MS: '20' };
delete env.ROBOCO_EDGE_TOKEN;
delete env.ROBOCO_ORG_ID;
const processes = [];
...
function stat(pid) {
  try {
    ...
    const pss = process.env.ROBOCO_PROFILE_PSS === '1'
...
try {
  ...
  const backgroundChats = Number(process.env.ROBOCO_PROFILE_BACKGROUND_CHATS ?? 0);
  ...
  const ui = start([`roboco://open/chat/${chatId}?workspace=${locator}`], 'ui', { ROBOCO_DATA_DIR: `${output}/ui` });
  ...
  if (!macOS && process.env.DISPLAY && !process.env.WAYLAND_DISPLAY) {
    await sleep(2000);
    const windows = execFileSync('xdotool', ['search', '--class', '^roboco$'], { encoding: 'utf8' }).trim().split('\n');
    if (windows.length !== 1) throw Error('Use a dedicated X display with exactly one Roboco window');
  ...
  phase = 'idle';
  await sleep(Number(process.env.ROBOCO_PROFILE_PRE_IDLE_MS ?? 10000));
  if (ui.exitCode != null) throw Error('UI exited; inspect ui.log');
  phase = 'stream';
  const prompt = process.env.ROBOCO_PROFILE_PROMPT ??
  ...
  const turns = Number(process.env.ROBOCO_PROFILE_TURNS ?? 1);
  if (!Number.isSafeInteger(turns) || turns < 1) throw Error('ROBOCO_PROFILE_TURNS must be a positive integer');
  for (let turn = 0; turn < turns; turn++) {
    const priorAssistants = new Set(transcript.filter(e => e.role === 'assistant').map(e => e.id));
    if (process.env.ROBOCO_PROFILE_SUBMIT_UI === '1') {
    ...
    const timeoutMs = Number(process.env.ROBOCO_PROFILE_TIMEOUT_MS ?? 240000);
  ...
  phase = 'settled';
  await sleep(Number(process.env.ROBOCO_PROFILE_IDLE_MS ?? 15000));
  if (streamError) throw streamError;
  ...
  const summary = { binary, binarySha256, harness, mode: process.env.ROBOCO_REPLAY_JOURNAL ? 'replay' : 'live',
    platform: process.platform, arch: process.arch,
    renderThreads: process.env.LP_NUM_THREADS ?? 'default', frameStats: env.ROBOCO_FRAME_STATS,
    submission: process.env.ROBOCO_PROFILE_SUBMIT_UI === '1' ? 'composer' : 'rpc', turns, backgroundChats, prompt,
  ...
  for (const name of ['engine', 'ui']) {
    const budget = process.env[`ROBOCO_MAX_${name.toUpperCase()}_RSS_MIB`];
  ...
  if (process.env.ROBOCO_PROFILE_HOLD_MS) await sleep(Number(process.env.ROBOCO_PROFILE_HOLD_MS));
} finally {
  clearInterval(sampler);
