...
public static class RobocoClientCapture {
...
[uint32]$owner = 0
[void][RobocoClientCapture]::GetWindowThreadProcessId($hwnd, [ref]$owner)
...
$previousDpi = [RobocoClientCapture]::SetThreadDpiAwarenessContext([IntPtr]::new(-4))
$bitmap = $null
try {
    $rect = [RobocoClientCapture+Rect]::new()
    if (-not [RobocoClientCapture]::GetClientRect($hwnd, [ref]$rect)) { throw 'GetClientRect failed' }
    $dpi = [RobocoClientCapture]::GetDpiForWindow($hwnd)
    ...
    try {
        ...
        if (-not [RobocoClientCapture]::PrintWindow($hwnd, $dc, 3)) { throw 'PrintWindow failed' }
    ...
    if ($previousDpi -ne [IntPtr]::Zero) { [void][RobocoClientCapture]::SetThreadDpiAwarenessContext($previousDpi) }
}
