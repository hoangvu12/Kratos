...
#   target/package/roboco-<version>-macos-<arch>.dmg          (user download)
#   target/package/roboco-<version>-macos-<arch>-app.tar.gz   (auto-updater)
# containing Roboco.app (unsigned unless CODESIGN_IDENTITY is set).
#
...
APP="$OUT_DIR/Roboco.app"
DMG="$OUT_DIR/roboco-$VERSION-macos-$ARCH.dmg"
APP_TARBALL="$OUT_DIR/roboco-$VERSION-macos-$ARCH-app.tar.gz"
...
install -m 755 "$ROOT/target/release/roboco" "$APP/Contents/MacOS/roboco"
...
# applied ahead of time; dist/roboco.png stays the full-bleed shared artwork).
ICONSET="$OUT_DIR/roboco.iconset"
...
iconutil -c icns "$ICONSET" -o "$APP/Contents/Resources/roboco.icns"
rm -rf "$ICONSET"
...
if $NOTARIZE; then
  ...
  # Gatekeeper offline.
  ZIP="$OUT_DIR/roboco-notarize.zip"
...
tar -czf "$APP_TARBALL" -C "$OUT_DIR" Roboco.app
...
dmgbuild.build_dmg(
    ...
    settings={
        ...
        "icon": os.path.join(app, "Contents/Resources/roboco.icns"),
        ...
        "icon_size": 104,
        "text_size": 12,
        "icon_locations": {"Roboco.app": (165, 195), "Applications": (495, 195)},
    },
)
PY
rm -f "$BG_TIFF"
