...
param(
    [string]$Exe = (Join-Path $PSScriptRoot '../target/release/roboco.exe'),
    [int]$Runs = 2
)
...
if (-not ('RobocoWindowProbe' -as [type])) {
Add-Type @'
using System;
using System.Text;
using System.Runtime.InteropServices;
public static class RobocoWindowProbe {
...
$overrides = @{
    ...
    ROBOCO_DATA_DIR = (Join-Path $root 'Roboco')
    ROBOCO_EDGE_TOKEN = $null; ROBOCO_IPC_PORT = '0'
    ROBOCO_EDGE_URL = 'http://127.0.0.1:1'; ROBOCO_ORG_ID = $null
    ROBOCO_HARNESS = 'mock'; RUST_BACKTRACE = '1'
}
$saved = @{}
...
try {
    ...
    foreach ($run in 1..$Runs) {
        ...
        try {
            ...
            [void][RobocoWindowProbe]::GetClassName($hwnd, $class, $class.Capacity)
            [uint32]$owner = 0
            [void][RobocoWindowProbe]::GetWindowThreadProcessId($hwnd, [ref]$owner)
            ...
            $device = (Get-Content -LiteralPath (Join-Path $root 'Roboco/device-id') -Raw).Trim()
