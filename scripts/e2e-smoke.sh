...
# roboco-rpc e2e_driver example proving the doc-queued cross-device command path:
#
#   B queues a Run into the chat doc -> nudge -> A (host) executes via the mock
#   harness -> transcript + session status sync A -> edge -> B.
#
# Both engines run as the SAME user (alice@org1) on different devices — roboco's
# one-user-many-devices model; chat/device rooms are claim-on-first-join per user.
#
# Usage: scripts/e2e-smoke.sh
# Env:   ROBOCO_E2E_EDGE_PORT (default 27640), ROBOCO_E2E_KEEP_LOGS=1 to keep logs.

set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
command -v cargo >/dev/null 2>&1 || PATH="$HOME/.cargo/bin:$PATH"
EDGE_PORT="${ROBOCO_E2E_EDGE_PORT:-27640}"
...
A_DIR=/tmp/e2e-a
B_DIR=/tmp/e2e-b
LOG_DIR="$(mktemp -d /tmp/roboco-e2e-logs.XXXXXX)"
...
cleanup() {
  ...
  if [[ "${ROBOCO_E2E_KEEP_LOGS:-0}" != "1" ]]; then
    rm -rf "$LOG_DIR"
  else
...
echo "build: roboco + e2e_driver"
# Two invocations: a single one with `--example` builds ONLY the example
# (the target filter applies across every -p), silently skipping the roboco
# bin — the smoke then dies on "No such file or directory".
(cd "$ROOT" && cargo build -q -p roboco)
(cd "$ROOT" && cargo build -q -p roboco-rpc --example e2e_driver)
ROBOCO="$ROOT/target/debug/roboco"
...
start_engine() { # start_engine <data_dir> <ipc_port> <name> <log>
  ROBOCO_DATA_DIR="$1" ROBOCO_IPC_PORT="$2" ROBOCO_DEVICE_NAME="$3" \
    ROBOCO_EDGE_URL="$EDGE_URL" ROBOCO_EDGE_TOKEN="$TOKEN" ROBOCO_ORG_ID="$ORG" \
    ROBOCO_HARNESS=mock RUST_LOG=info \
    "$ROBOCO" headless >"$4" 2>&1 &
}
